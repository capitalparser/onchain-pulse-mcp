...
이 프로젝트는 위 신호들을 **MCP 도구로 노출하는 read-only / stateless 서버**다. 첫 릴리스는 거시 mood (D view) 에 집중하고, 이후 종목 스크리닝 (B), 진입·청산 타이밍 (A) view 를 같은 데이터 레이어 위에 단계적으로 추가한다.
...
- **G1**. D view 거시 mood 신호를 6개 MCP tool로 노출
...
- **G5**. composite mood score 계산식·가중치를 `config/mood.yaml` 로 외부화
...
- **P2. 기관 분석가 / 펀드 PM**: BYOK 키 (Nansen, Glassnode 등) 보유. 매일 아침 mood 진단 자동화. 자체 cron + Slack/Telegram 자체 구축.
...
2. **Adapter plug-in**: source 별 독립 모듈. BYOK 키 detect 시 enriched, 없으면 free fallback (graceful degradation)
3. **알림은 MCP 밖**: cron/룰/봇은 사용자가 self-host. 본 repo는 reference YAML 룰만 `examples/rules/` 에 commit
4. **합성 지표 투명**: composite mood score 식·가중치 모두 코드/config로 공개 — 신뢰 = 투명성
5. **도구 bundling**: MCP tool은 의사결정 단위 (예: `get_market_mood()` 한 번 호출 → 6개 어댑터 fan-out). 에이전트의 호출 횟수 최소화
...
| `kr_layer` | Upbit public, Bithumb public | (선택) | 김프 spread, Upbit netflow, KRW 거래량 |
...
interface Adapter<T> {
  name: string;
  ttlMs: number;
  capabilities(): { enriched: boolean; sources: string[] };
...
```typescript
get_market_mood()            // 합성: mood score 0~100 + verdict + raw inputs
...
```json
{
  "summary": "ETF +$340M 7일 누적, mood: risk-on (78/100)",
  "score": 78,
  "verdict": "risk-on",
...
- `verdict`: `risk-off` (0–30) / `neutral` (30–70) / `risk-on` (70–100) / `unknown` (모든 source down).
- `confidence`: 입력 누락 시 1.0 미만 (살아남은 weight 합 / 전체 weight).
- `stale_data`: rate limit·source down으로 fallback한 항목 명시.
- `capabilities.enriched`: BYOK 키로 paid endpoint 사용했는지.

### 5.4 Composite mood score 합성식 (`config/mood.yaml`)
...
| `kr_layer` | 5분 | Upbit rate limit 보호 |
...
어댑터가 시작 시 detect, `capabilities.enriched` 메타에 반영. 없으면 free fallback. 사용자 BYOK 가이드는 `README.md` 별도 섹션.

## 6. Data flow

### 6.1 On-demand path (B 시나리오 — 에이전트 쿼리)

1. Agent → MCP tool call (예: `get_market_mood()`)
2. Dispatcher → adapter들에 `Promise.all` parallel fan-out
3. 각 adapter: 캐시 hit? → 반환 / miss → BYOK 또는 free endpoint hit
4. Aggregator: weighted z-score → sigmoid → score → verdict bucket
5. Response: `{ summary, score, verdict, inputs, sources, stale_data, confidence }`
...
| 상황 | 응답 |
|---|---|
| BYOK 키 없음 | free fallback. `capabilities.enriched: false` |
...
| 모든 source down | `summary: "data unavailable"`, `score: null`, `verdict: "unknown"` (에러 throw 안 함) |
| 잘못된 input | MCP tool schema validation reject (`zod`) |
| network timeout | 어댑터별 5초 timeout → cache fallback → 그래도 없으면 단일 source down 처리 |

**에러 throw 정책**: tool 응답은 가능한 항상 200 + `verdict: "unknown"` 으로 graceful degradation. 에이전트가 자연어로 처리 가능하게.
...
- **Q3**. mood score `funding reverse` 임계 — `|z| > 2` 가 적정한지 backtest 필요. v0.1은 임의값으로 시작, v0.2에서 historical 검증.
- **Q4**. RWA TVL 7d delta 의 가중치 0.05 — 너무 작은가? 거시 mood에서 RWA가 가지는 의미는 "전통금융 자본 유입" proxy. 사용자가 thesis에 따라 config로 조정.
...
- **v0.5**: composite score backtesting harness — 과거 데이터로 mood score vs 실제 시장 움직임 검증.
