...
function quotes(): any[] {
  return [
    ...
    { ...expectedStrategies[4], underlyingToken: expectedStrategies[4].underlying_token, shareAccountingTokenAmount: 600n, tokenCustodyTokenAmount: 570n, directShareAccountingEthQuote: 650n, directTokenCustodyEthQuote: 620n },
    { ...expectedStrategies[5], underlyingToken: expectedStrategies[5].underlying_token, shareAccountingTokenAmount: 500n, tokenCustodyTokenAmount: 470n, directShareAccountingEthQuote: 555n, directTokenCustodyEthQuote: 521n },
  ];
}
function build(candidate = quotes(), lsethLastCompletedEpochId = 123n) {
  return buildVerifiedEigenLayerLstEthQuotesSnapshot({ block, quotes: candidate, lsethLastCompletedEpochId, sources: ["ethereum_rpc"], sourceStatus });
}

describe("EigenLayer covered LST ETH quote metrics", () => {
  it("computes exact six-token independent/direct/floor quotes and partial sums", () => {
  ...
  it("keeps stale and unavailable evidence bounded", () => {
    const stale = buildVerifiedEigenLayerLstEthQuotesSnapshot({ block, quotes: quotes(), lsethLastCompletedEpochId: 123n, sources: ["ethereum_rpc"], sourceStatus, stale: true });
