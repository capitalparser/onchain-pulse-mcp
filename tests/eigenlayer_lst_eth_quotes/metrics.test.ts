...
describe("EigenLayer covered LST ETH quote metrics", () => {
  it("computes nine ordered quotes with nominal OETH identity and only partial sums", () => {
    ...
    expect(snapshot.methodology).toBe("eigenlayer-covered-lst-eth-quotes-v6");
  ...
  it("accepts zero rebase, paused true, and delay zero as context only", () => {
    const snapshot = build();
    expect(snapshot.report_context).toMatchObject({ oeth_last_rebase_unix: "0", oeth_rebase_paused: true, oeth_withdrawal_claim_delay_seconds: "0" });
    expect(snapshot.summary).toMatch(/nominal OETH unit accounting/);
  });
  ...
  it("keeps stale v6 and unavailable evidence bounded", () => {
    ...
    expect(unavailable).toMatchObject({ methodology: "eigenlayer-covered-lst-eth-quotes-v6", status: "unavailable", verified_block: null, covered_quotes: [], report_context: null });
  });
});
