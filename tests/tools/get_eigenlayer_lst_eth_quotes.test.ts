...
describe("getEigenLayerLstEthQuotes", () => {
  it("localizes and sanitizes verified covered quotes without widening the measurement", () => {
    ...
    expect(result.summary).toContain("Finalized direct protocol-accounting quotes for stETH/rETH/cbETH/osETH/mETH cover only 5 of 12 fixed strategies");
    expect(result.summary).toContain("share-accounting and custody sums are distinct partials");
  ...
  it("localizes verified, stale, and unavailable evidence in English and Korean with every boundary explicit", () => {
    ...
    for (const result of cases) {
      ...
      expect(result.summary).toMatch(/5 of 12 fixed strategies|고정 전략 12개 중 5개/);
      expect(result.summary).toMatch(/share-accounting and custody sums are distinct partials|지분 회계와 토큰 보관 합계는 서로 다른 부분 합계/);
      expect(result.summary.length).toBeLessThanOrEqual(500);
      expect(JSON.stringify(result)).not.toContain("credential-secret");
    }
    ...
    for (const result of cases.slice(0, 3)) {
      ...
      expect(result.summary).toMatch(/cbETH\/osETH reward\/mETH oracle-record freshness/);
    ...
    for (const result of cases.slice(3)) {
      ...
      expect(result.summary).toMatch(/독립 담보/);
      expect(result.summary).toMatch(/cbETH\/osETH 보상\/mETH 오라클 기록 최신성/);
