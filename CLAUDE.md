...
- `@modelcontextprotocol/sdk`, `zod`, `lru-cache`, `yaml`
- `vitest` (testing), `tsup` (bundle)
- Distribution: npm (`npx onchain-pulse-mcp`)

## Domain

See `CONTEXT.md` (seeded by `/grill-with-docs`) and `docs/adr/0001-domain.md` once those steps complete.

## Workflow gate

Implementation is delegated to **Codex CLI** through `HANDOFF.md`. Claude (Opus) responsibilities are limited to: design spec, plan, scaffold, ADRs, cross-model reviews, merge decisions. Claude must not implement Tasks 2–23 of `plans/2026-05-08-onchain-pulse-mcp.md` directly.

## Documents

- `plans/2026-05-08-onchain-pulse-mcp.md` — 23-task TDD plan (source of truth for impl)
